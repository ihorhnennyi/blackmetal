...
import { CommonButtonStyles, CommonTextStyles } from '../../utils';
...
const MonoCard = ({ image, title, description, link }: MonoCardProps) => {
  ...
  const getFileExtensionFromUrl = (url: string): string => {
    if (!url) return 'docx';
    const parts = url.split('.');
    return parts[parts.length - 1].toLowerCase() || 'document';
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          ) : ['pdf', 'jpg', 'svg', 'jpeg', 'png', 'webp', 'jfif'].includes(fileExtension) ? (
            <>
              <MUILink
                target="_blank"
                href={fileLink}
